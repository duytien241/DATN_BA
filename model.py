...
class Attention(nn.Module):
    ...
    def forward(self, dec_output, enc_output, mask):
        ...
        attn_w = F.softmax(attn_w.view(-1, enc_seq_len),
                           dim=1).view(batch_size, -1, enc_seq_len)
        ...
        output = torch.tanh(self.linear(
            combined.view(-1, 2*dec_hidden_size))).view(batch_size, -1, dec_hidden_size)
...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab, max_len, hidden_size, n_layers,
        ...
        self.softmax_masks = torch.ones(
            self.vocab_size, self.vocab_size).to(device)

        # create teacher forcing masks
        self.teacher_forcing_masks = torch.ones(
            self.vocab_size).long().to(device)
        ...
        for idx in range(self.vocab_size):
            ...
            if token in AEIOUYD_VN:
                ...
                token_indices = self.aeiouyd_indices[unidecode.unidecode(
                    token)]
    ...
    def forward(self, targets, enc_output, enc_hidden, mask, mask_softmax=0.5, teacher_forcing=0.5):
        ...
        if self.reverse:
            ...
            enc_output = torch.cat(
                (enc_output[:, :, (self.hidden_size//2):], enc_output[:, :, :(self.hidden_size//2)]), dim=2)
        ...
        dec_probs = torch.zeros(batch_size, max_len,
                                self.vocab_size).to(device)

        # tensor to store decoder lstm output
        dec_outputs = torch.zeros(
            batch_size, max_len, self.hidden_size).to(device)

        # tensor to store softmax masks for entire seq_len
        softmax_masks = torch.ones(
            batch_size, max_len, self.vocab_size).to(device)
        _, first_softmax_mask = self._check_token_(
            targets[:, 0], mask_softmax=mask_softmax, teacher_forcing=teacher_forcing)
        ...
        for t in range(1, max_len):
            ...
            is_aeiouyd, softmax_mask = self._check_token_(
                targets[:, t], mask_softmax=mask_softmax, teacher_forcing=teacher_forcing)
            ...
            if teacher_forcing > 0.0:
                dec_input = targets[:, t] * is_aeiouyd + \
                    predicted * (1 - is_aeiouyd)
    ...
    def _check_token_(self, indices, mask_softmax=0.5, teacher_forcing=0.5):
        ...
        teacher_forcing_mask = torch.index_select(
            self.teacher_forcing_masks, 0, indices)

        # apply softmax mask ratio
        if 0.0 <= mask_softmax < 1.0:
            softmax_mask_ratio = torch.rand(batch_size, 1, device=device).lt(
                mask_softmax).repeat(1, self.vocab_size).float()
            softmax_mask = softmax_mask.mul(softmax_mask_ratio)

        # apply teacher forcing ratio
        if 0.0 <= teacher_forcing < 1.0:
            teacher_forcing_ratio = torch.rand(
                batch_size, device=device).lt(teacher_forcing).long()
            teacher_forcing_mask = teacher_forcing_mask.mul(
...
class Seq2SeqConcat(nn.Module):
    ...
    def __init__(self, encoder, decoder, reverse_decoder, pad_idx=1):
        ...
        self.linear = nn.Linear(
            self.decoder.hidden_size * 2, self.decoder.vocab_size)
    ...
    def forward(self, batch, has_targets=True, mask_softmax=0.5, teacher_forcing=0.5):
        ...
        if has_targets:
            ...
            for i in range(len(self.encoder.vocab)):
                targets[targets2 ==
                        i] = self.decoder.vocab.stoi[self.encoder.vocab.itos[i]]

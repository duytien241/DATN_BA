...
class Predictor:
    def __init__(self, max_len=1000, hidden_size=300, n_layers=2):
        # load vocab
        ...
        # prepare model
        self.encoder = Encoder(self.src_vocab, self.max_len,
                               self.hidden_size, self.n_layers)
        self.decoder = Decoder(self.tgt_vocab, self.max_len,
                               self.hidden_size * 2, self.n_layers)
        self.reverse_decoder = Decoder(
            self.tgt_vocab, self.max_len, self.hidden_size * 2, self.n_layers, reverse=True)
        self.model = Seq2SeqConcat(
            self.encoder, self.decoder, self.reverse_decoder, self.pad_idx)
        ...
        with open("data/legal_vc.txt", 'r', encoding='utf8') as f:
            for line in f:
                vc = line.strip()
    ...
    def predict(self, message):
        ...
        lines_id = ['000']
        lines_raw = [message]
        lines_cleaned = [message]
        lines_prep = [self.preprocess(message)]
        # prepare dataset
        ...
        # predict
        with open(out_path, 'w', encoding='utf8') as writer:
            with torch.no_grad():
                for i, batch in enumerate(test_iterator):
                    # forward through model
                    _, _, output = self.model(
                        batch, has_targets=False, mask_softmax=1.0, teacher_forcing=1.0)

                    # get top-1
                    predicted_values, predicted_indices = torch.max(
                        output, dim=-1)
                    ...
                    predicted_seq = self.match_output(
                        predicted_seq, lines_cleaned[i])
    ...
    def beam_lm(self, predicted_seq, predicted_values, line_raw, k=500, threshold=0.99):
        ...
        for i, v in enumerate(predicted_values):
            if v < threshold:
                predicted_seq_copy = predicted_seq_copy[:i] + \
                    '*' + predicted_seq_copy[i+1:]
            elif v < 1.0:
                predicted_seq_copy = predicted_seq_copy[:i] + \
                    '~' + predicted_seq_copy[i+1:]

        predicted_seq_copy = re.sub(
            '~*\\*+~*', lambda x: '*'*len(x.group()), predicted_seq_copy)

        predicted_seq_copy = ''.join(
            [predicted_seq[i] if c == '~' else c for i, c in enumerate(predicted_seq_copy)])
        ...
        def beam_lm_(predicted_seq, predicted_values, predicted_seq_uncertain, k=k, step=0):
            ...
            uncertainties = [m.span() for m in re.finditer(
                '\\*+', predicted_seq_uncertain)]
            ...
            for i, (start_idx, end_idx) in enumerate(uncertainties):
                ...
                c_list = self.tone_map.get(unidecode.unidecode(c), [
                                           unidecode.unidecode(c)])

                # get contexts
                left_contexts = topk_fwd
                right_context = predicted_seq_uncertain[end_idx:uncertainties[i + 1][0]] if i < len(
                    uncertainties) - 1 else predicted_seq_uncertain[end_idx:]

                candidates = []
                scores = torch.empty(
                    len(c_list) * len(left_contexts)).fill_(-float('inf'))
                ...
                for ch in c_list:
                    for left_context in left_contexts:
                        ...
                        score = self.score(
                            self.match_punct(candidate, line_raw))
                ...
                if len(candidates) > 0:
                    _, topk_fwd_scores = torch.topk(
                        scores, k=min(k, len(candidates)))
                    current_topk_fwd = [candidates[s]
                                        for s in topk_fwd_scores.tolist()]
                    if len(current_topk_fwd) > 0:
                        topk_fwd = current_topk_fwd
                    else:
                        topk_fwd = [
                            left_context + unidecode.unidecode(c) + right_context for left_context in left_contexts]
                else:
                    topk_fwd = [
                        left_context + unidecode.unidecode(c) + right_context for left_context in left_contexts]
            ...
            for i, (start_idx, end_idx) in reversed(list(enumerate(uncertainties))):
                ...
                candidates = []
                scores = torch.empty(
                    len(c_list) * len(right_contexts)).fill_(-float('inf'))

                j = 0
                for ch in c_list:
                    for right_context in right_contexts:
                        candidate = left_context + ch + right_context
                        score = self.score(self.match_punct(
                            candidate, line_raw, forward=False))
                        candidates.append(candidate)
                        scores[j] = score
                        j += 1

                if len(candidates) > 0:
                    _, topk_bwd_scores = torch.topk(
                        scores, k=min(k, len(candidates)))
                    current_topk_bwd = [candidates[s]
                                        for s in topk_bwd_scores.tolist()]
    ...
    def match_output(self, predicted, cleaned):
        ...
        try:
            for predicted_ptr in range(len(predicted)):
                if unidecode.unidecode(predicted[predicted_ptr]) == unidecode.unidecode(cleaned[cleaned_ptr]):
    ...
    def preprocess(self, line):
        ...
        line = ''.join(c if c not in PUNCT else '-' for c in line)
        # replace unknown characters with '?'
        line = ''.join(c if c in LEGAL else '?' for c in line)
        return line
